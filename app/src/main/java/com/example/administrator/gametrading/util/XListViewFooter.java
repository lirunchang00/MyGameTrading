...
public class XListViewFooter extends LinearLayout{
    ...
    public void setState(int state) {
        ...
        if (state == STATE_READY) {
            ...
            mHintView.setText(R.string.lv_footer_hint_ready);
        } else if (state == STATE_LOADING) {
            mProgressBar.setVisibility(View.VISIBLE);
        } else {
            mHintView.setVisibility(View.VISIBLE);
            mHintView.setText(R.string.lv_footer_hint_normal);
        }
    }
    ...
    private void initView(Context context) {
        mContext = context;
        LinearLayout moreView = (LinearLayout) LayoutInflater.from(mContext).inflate(R.layout.footview, null);
        addView(moreView);
        moreView.setLayoutParams(new LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));

        mContentView = moreView.findViewById(R.id.lv_footer_content);
